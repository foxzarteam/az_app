...
class _WalletScreenState extends State<WalletScreen> {
  ...
  Future<void> _loadPaymentDetails() async {
    ...
    if (!mounted) return;
    await wc.refresh(mobile);
    if (!mounted) return;
    setState(() {
      _userId = wc.userId;
      _savedUpi = wc.savedUpi;
      _savedBankName = wc.savedBankName;
      _savedIfscCode = wc.savedIfscCode;
      _balanceDisplay = wc.balanceDisplay;
      final upi = wc.savedUpi;
      if (upi != null && upi.isNotEmpty) _upiController.text = upi;
      final bn = wc.savedBankName;
      final ifsc = wc.savedIfscCode;
      if (bn != null && bn.isNotEmpty) _bankNameController.text = bn;
...
class _UpiSheetContent extends StatelessWidget {
  ...
  String _tr(BuildContext context, String key) =>
      Provider.of<AppLocale>(context, listen: false).t(key);
              ...
              child: ElevatedButton(
                onPressed: userId == null
                    ...
                    : () async {
                        ...
                        if (err != null) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text(_tr(context, err), style: GoogleFonts.inter()), backgroundColor: AppTheme.error),
                          );
                          return;
                        }
                        final ok = await context
                            .read<WalletController>()
                            .saveUpiAccount(userId!, upiId);
                        if (!context.mounted) return;
                        if (ok) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text(_tr(context, 'msgUpiSaved'), style: GoogleFonts.inter()), backgroundColor: AppTheme.success),
                          );
                          onSaved();
                        } else {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text(_tr(context, 'msgErrorTryAgain'), style: GoogleFonts.inter()), backgroundColor: AppTheme.error),
                          );
                        }
                      },
...
class _BankSheetContent extends StatelessWidget {
              ...
              child: ElevatedButton(
                onPressed: userId == null
                    ...
                    : () async {
                        ...
                        if (bankName.isEmpty) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text(_tr(context, 'msgEnterBankName'), style: GoogleFonts.inter()), backgroundColor: AppTheme.error),
                          );
                          return;
                        }
                        ...
                        if (ifscErr != null) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text(_tr(context, ifscErr), style: GoogleFonts.inter()), backgroundColor: AppTheme.error),
                          );
                          return;
                        }
                        final ok = await context
                        ...
                        if (ok) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text(_tr(context, 'msgBankSaved'), style: GoogleFonts.inter()), backgroundColor: AppTheme.success),
                          );
                          onSaved();
                        } else {
                          ScaffoldMessenger.of(context).showSnackBar(
  ...
  InputDecoration _inputDecoration({String hint = 'e.g. State Bank of India'}) {
    return InputDecoration(
      ...
      enabledBorder: OutlineInputBorder(
        borderRadius: BorderRadius.circular(12),
        borderSide: BorderSide(color: AppTheme.primaryBlue.withValues(alpha: 0.25)),
      ),
      focusedBorder: OutlineInputBorder(
        borderRadius: BorderRadius.circular(12),
